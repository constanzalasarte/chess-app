...
class ClassicEngine: Engine {
    override fun pawnEngine(): ValidationEngine {
        ...
        val pieceMove0 : ValidationEngine = GraphNode(QuantityOfPieceMove(0), listOf())
        val verticalMove2 : ValidationEngine = GraphNode(QuantityOfMoves(2, MoveType.VERTICAL), listOf(pieceMove0))
        val verticalMove1 : ValidationEngine = GraphNode(QuantityOfMoves(1, MoveType.VERTICAL), listOf())
    ...
    override fun horseEngine(): ValidationEngine{
        val noFriendInDestinationSquare : ValidationEngine = GraphNode(CheckDestinationSquare(), listOf())
    ...
    override fun rookEngine(): ValidationEngine {
        val obsHorizontalMove: ValidationEngine = GraphNode(ObsHorizontalMove(), listOf())
        val horizontalMove: ValidationEngine = GraphNode(HorizontalMove(), listOf(obsHorizontalMove))

        val obsVerticalMove: ValidationEngine = GraphNode(ObsVerticalMove(), listOf())
        val verticalMove: ValidationEngine = GraphNode(VerticalMove(), listOf(obsVerticalMove))
    ...
    override fun queenEngine(): ValidationEngine {
        ...
        val obsDiagonalMove : ValidationEngine = GraphNode(ObsDiagonalMove(), listOf())
        val diagonalMove : ValidationEngine = GraphNode(DiagonalMove(), listOf(obsDiagonalMove))
    ...
    override fun kingEngine(): ValidationEngine {
        ...
        val validateCastle : ValidationEngine = GraphNode(ValidateCastle(), listOf())
    ...
    private fun getQuantityMoveGraph(quantity: Int, moveType: MoveType, list: List<ValidationEngine> = listOf()): ValidationEngine{
